$(function() {
    ...
    App.Models.Hello = Backbone.Model.extend({
        url: function() {
            return 'api/custom.html'; // 获得数据的后台地址。  
        },
        initialize: function() {
            this.set({
                'message': 'hello world'
            }); // 前端定义一个message字段，name字段由后端提供。  
        }
    });

    App.Models.Demo = Backbone.Model.extend({
        initialize: function() {
            this.set({
                'message': 'demo',
				'name': 'maocg'
    ...
    App.Controllers.Routes = Backbone.Router.extend({
        ...
        demo: function() {
			var demoModel = new App.Models.Demo;
			var demoView = new App.Views.Demo({
				model: demoModel
			});
			demoView.trigger('change');
        },		
    });
